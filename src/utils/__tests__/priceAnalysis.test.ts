...
describe('Price Analysis Utils', () => {
  ...
  describe('findSupportResistanceLevels', () => {
    ...
    it('should return sorted levels', () => {
      ...
      for (let i = 0; i < levels.resistance.length - 1; i++) {
        expect(levels.resistance[i]).toBeLessThanOrEqual(
          levels.resistance[i + 1]
        )
      }
    })
  })
  ...
  describe('calculateEntryPoints', () => {
    it('should calculate entry points with different strategies', () => {
      ...
      expect(entryPoints.conservative).toBeGreaterThanOrEqual(
        entryPoints.moderate
      )
      expect(entryPoints.moderate).toBeGreaterThanOrEqual(
        entryPoints.aggressive
      )
    })
  ...
  describe('analyzePricePoints', () => {
    ...
    it('should adjust analysis based on time horizon', () => {
      const shortTermAnalysis = analyzePricePoints(
        mockOHLCVData,
        mockCurrentPrice,
        '5m'
      )
      const longTermAnalysis = analyzePricePoints(
        mockOHLCVData,
        mockCurrentPrice,
        '1d'
      )
