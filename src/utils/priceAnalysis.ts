...
export function calculateEntryPoints(
  ...
  if (levels.support.length > 0) {
    const nearestSupport =
      levels.support.find((s) => s < currentPrice) || levels.support[0]
...
export function calculateStopLoss(
  ...
  switch (method) {
    ...
    case 'support': {
      const levels = findSupportResistanceLevels(data)
      const nearestSupport =
        levels.support.find((s) => s < entryPrice) || levels.support[0]
...
export function calculateProfitTargets(
  data: OHLCV[],
  entryPrice: number,
  ...
  if (target1 - entryPrice < entryPrice * 0.02) {
    // Less than 2% gain
    target1 = entryPrice * 1.05 // 5% above entry
    target2 = entryPrice * 1.1 // 10% above entry
  }
  ...
  const fibExtensions = calculateFibonacciExtensions(
    recentHigh,
    recentLow,
    entryPrice
  )
  ...
  if (target3 - target2 < entryPrice * 0.03) {
    // Less than 3% between targets
  ...
  if (fibExtensions.targets.length > 0) {
    ...
    if (fib127 && fib127.price > target2 && fib127.price > target3) {
      ...
      fib161 &&
      fib161.price > target2 &&
      fib161.price < target3 * 1.5
    ) {
  ...
  return {
    ...
    riskRewardRatio: isFinite(riskRewardRatio)
      ? Math.round(riskRewardRatio * 10) / 10
      : NaN,
    methods: {
...
export function analyzePricePoints(
  data: OHLCV[],
  currentPrice: number,
  timeHorizon: TimeInterval
): PriceAnalysis {
  ...
  const levels = findSupportResistanceLevels(data)
  confidence += Math.min(
    (levels.support.length + levels.resistance.length) / 20,
    0.2
  ) // Up to +0.2
  ...
  if (['5m', '15m'].includes(timeHorizon)) {
    riskAssessment =
      'High - Short timeframe with increased volatility and noise'
