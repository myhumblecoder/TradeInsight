...
describe('Article Component - Enhanced with Markdown', () => {
  ...
  it('should render plain text when not enhanced', () => {
    const plainText =
      'Bitcoin is trading at $50,000. This is a simple analysis.'
  ...
  it('should render plain text when enhanced but no markdown detected', () => {
    const plainText =
      'Bitcoin is trading at $50,000. Simple analysis without markdown.'
...
> This analysis suggests a positive outlook for Bitcoin.`
  ...
  it('should show confidence score for all variants', () => {
    ...
    testCases.forEach(({ text, isEnhanced, confidence }) => {
      render(
        <Article text={text} confidence={confidence} isEnhanced={isEnhanced} />
      )
      expect(
        screen.getByText(`Confidence Score: ${confidence}%`)
      ).toBeInTheDocument()
    })
  })
  ...
  it('should render blockquotes in markdown', () => {
    const markdownWithBlockquote =
      '> This is an important market insight from our analysis.'

    render(
      <Article
        text={markdownWithBlockquote}
        confidence={82}
        isEnhanced={true}
      />
    )
...
2. Second priority action`
    ...
    render(
      <Article text={markdownWithLists} confidence={78} isEnhanced={true} />
    )
