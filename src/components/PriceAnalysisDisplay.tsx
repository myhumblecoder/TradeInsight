...
export function PriceAnalysisDisplay({
  ...
  const PriceEntryWithTooltip = ({
    ...
    <div className="flex justify-between items-center">
      ...
      >
        <span
          className={`${colorClass} flex-shrink-0 cursor-help flex items-center`}
        >
  ...
  return (
    ...
    >
      ...
      <div
        className={`grid gap-6 ${compact ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}
      >
        {/* Entry Points */}
        ...
        <div className="space-y-3">
          ...
          <Tooltip
            content={
              analysis.stopLoss.explanation ||
              'Stop loss level calculated based on technical analysis'
            }
            position="right"
          >
            <div className="text-red-600 dark:text-red-400 cursor-help flex items-center">
              {formatPrice(analysis.stopLoss.price)} (
              {formatPercentage(analysis.stopLoss.percentage)} below entry)
              <svg
                className="w-3 h-3 ml-1 opacity-60"
                fill="currentColor"
                viewBox="0 0 20 20"
